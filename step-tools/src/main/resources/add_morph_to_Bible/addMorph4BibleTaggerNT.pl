...
# When the count of a Strong word is different in the DB and the verse, the tag has to be added manually.
	...
	OUTER: for my $strongNum (sort keys %strongCount) {
		...
		if (($countOfSameStrongInDB > 1) && ($countOfSameStrongInDB != $strongCount{$strongNum})) {
			if ($countOfSameSStrongInstance != $strongCount{$strongNum}) {
	...
	return $strongsToSkip . " ;" . $strongsWithDifferentOccurrences . " ";
}

sub printError {
	my $strongNum = shift;
	my $errorMsg = shift;
	my $posOfCheckStrong = index(" G0846 G1510 G3588 G3778 G5101 G1096 G2532 G1473 G3739 ", " " . $strongNum . " ");
			...
			if ($posOfQuote > -1) {
                ...
                foreach (@strongsInWord) {
					...
					if (index($strongsToSkip, $currentStrong) == -1) {
						...
						my $morph = findMorph($ref, $currentStrong, $strongCount{$currentStrong}, $stringAfterStrongNum, $useAltCount);
						if ($morph eq "") {
							my $glossToPrint = "";
    ...
    if (exists($altStrongMe{$strong})) {
   	...
   	for my $i (0 .. $#foundAltStrongs) {
        #print "recursion $strong,$ref, $foundAltStrongs[$i], $occurrenceInVerse, $stringAfterStrongNum, $useAltCount\n";
   		my $result = findMorph($ref, $foundAltStrongs[$i], $occurrenceInVerse, $stringAfterStrongNum, $useAltCount);
		if ($result ne "") {
			return $result;
		}
    }
