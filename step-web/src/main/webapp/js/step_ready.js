(function () {
    ...
    function checkForExampleURL() {
        ...
        if (urlVars.indexOf("colorCode1") > -1)
            ...
            cf.setNextPageURL('/?q=version=KJV@reference=Eph.1&options=HVGUNC&skipwelcome' + debugParam, 'color based on verb aspect', '');
        else if (urlVars.indexOf("colorCode5") > -1)
            cf.setNextPageURL('/?q=version=SBLG@reference=Rom.12&options=CEMVALHUN&skipwelcome' + debugParam, 'color based on verb aspect', '');
        else if (urlVars.indexOf("colorCode6") > -1)
            cf.setNextPageURL('/?q=version=THOT@reference=Gen.1&options=HVLUNC&skipwelcome' + debugParam, 'color based on verb aspect', '');
        else if (urlVars.indexOf("colorCode7") > -1)
            cf.setNextPageURL('/?q=version=CUn@reference=Col.1&options=HVGUNC&skipwelcome' + debugParam, 'color based on verb aspect', '');
        else if (urlVars.indexOf("colorCode8") > -1)
            cf.setNextPageURL('/?q=version=SBLG@version=KJV@version=CUn@reference=Eph.5&options=CVLHUVNEAM&display=INTERLEAVED&skipwelcome' + debugParam, 'color based on verb aspect', '');
