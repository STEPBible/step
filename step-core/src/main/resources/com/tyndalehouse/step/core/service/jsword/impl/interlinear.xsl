 ...
 * @author Chris Burrell [chris at burrell dot me dot uk] 
 ...
 <xsl:stylesheet
  ...
  <xsl:param name="fr_Vocab" select="'false'"/>
      ...
      <xsl:choose>
        <xsl:when test="$TinyVNum = 'true' and $Notes = 'true'">
          <span class="w verseStart">
			...
			<xsl:if test="$fr_Vocab = 'true'">
				<span class="strongs">Vocabulaire</span>
			</xsl:if>
        ...
        <xsl:when test="$TinyVNum = 'true' and $Notes = 'false'">
          <span class="w verseStart">
			...
			<!-- output a filling gap for strongs -->
              ...
              <xsl:if test="$fr_Vocab = 'true'">
                  <span class="text"><span class="smallHeaders strongs">vocabulaire</span></span>
              </xsl:if>
              <xsl:if test="$zh_tw_Vocab = 'true'">
			...
			<span class="{$classes} {$colorClass}" strong="{$lemma}" morph="{@morph}" var="{$variant}">
				<xsl:if test="normalize-space($remainingText) != ''">
					...
					<xsl:if test="$fr_Vocab = 'true'">
						<span class="strongs">
								<xsl:value-of
									select="vocab:get_fr_Vocab($vocabProvider, $baseVersion, ./ancestor::*[@osisID]/@osisID ,@lemma)" />
						</span>
					</xsl:if>
					<xsl:if test="$zh_tw_Vocab = 'true'">
						<span class="strongs">
