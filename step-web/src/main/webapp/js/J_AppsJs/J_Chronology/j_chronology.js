...
import {
  ClassJFrameworkMultiframeCommunicationsSlave
} from '/js/J_AppsJs/J_Framework/j_framework.multiframeCommunicationsSlave.js';
...
class _ClassJChronology extends ClassJFrameworkMultiframeCommunicationsSlave
{
  ...
  _makeInitialSelection ()
  {
    ...
    let year = urlParms.get('year') ?? '6BC'; // Date Jesus born.
    let showWelcome = urlParms.has('showWelcome');
    let type = urlParms.get('type') ?? 'event';
...
class _ClassJRenderer
{
  ...
  adjustDimensionsAndPositions (colWidthsOnly = false)
  {
    ...
    _ClassCommon.getColumnsContainer().style.gridTemplateColumns = widths.join('px ') + 'px 1fr';
  ...
  doDurationHeaders ()
  {
    ...
    function renderHeaderForItem (durationLineDiv)
    {
      ...
      let colour = getComputedStyle(durationLineDiv).backgroundColor;
  ...
  _addTickMarks ()
  {
    ...
    let year = Math.floor( (1 + JChronologyData.getFirstUYearBc()) / C_BcInterval) * C_BcInterval;
    while (year >= 0)
    {
  ...
  */
  ...
  _renderYears ()
  {
    ...
    /**********************************************************************/
    let mostRecentUnifiedYear = -1;
    let mostRecentPos = -1;
    const durationMap = new Map();
    let rawPositionFunction = null;
    const descriptionDivs = [];
    let associatedChapterIx = -1;
    ...
    const doYear = (e, type) =>
    {
      ...
      if (isVisibleEntry)
      {
        ...
        if (isChapterEntry)
          content = '<span class="iconFont">K&nbsp;</span>' + _ClassCommon.convertToRanges(JChronologyData.getChapterRefs(e)); // $$$Jamie 26-Aug-2026: Moved convertToRanges to _ClassCommon.
        else if (isAnnotatedYearEntry)
        {
          ...
          let scriptureRefs = JChronologyData.getChaptersFromChapterAndYearDataAsString(e);
          if ('' !== scriptureRefs)
          {
            scriptureRefs = _ClassCommon.convertToRanges(scriptureRefs); // $$$Jamie 26-Aug-2026: Moved convertToRanges to _ClassCommon.
            const x = scriptureRefs.split('; ');
            if (x.length > 1) scriptureRefs = x[0] + ' etc';
            content +=
...
class _ClassAlternativeChronologies
{
  ...
  {
    let s = chronologyName.replace('dt_', '');
    if (s.includes('_'))
    {
      let x = s.split('_');
  ...
  static _updateDateColumn (cacheKey)
  {
    ...
    for (const tag of tags)
    {
      ...
      let revisedContent = JChronologyData.getField(JChronologyData.getEntryGivenKey(elementKey), cacheKey);
...
class _ClassJChronologyPresentationHandler
{
  ...
  _copyLinkToClipboard ()
  {
    ...
    /* 26-Aug-2026: $$$Jamie.
       ...
       Mystifying ... Originally I had just
  
          navigator.clipboard.writeText(url);
  
       here.  Other applications (like genenalogy) also do this, and it
       works for them.  Here, however, it was failing.
  
       The code block below was recommended by Claude as an alternative
       to achieve the same end.  Apparently this relies upon a now
       deprecated approach, but Claude says it still works in all
       browsers, and the more 'official' alternatives (such as using
       window.focus() to force the document to have focus before
       attempting the copy are themselves blocked in some browsers
       (including the one I was using for text purposes). */
  ...
  _setInfoBoxContentForChapterEvent (entry)
  {
    ...
    const headerText = `<span class='iconFont'>K&nbsp;</span>${_ClassCommon.convertToRanges(JChronologyData.getChapterRefs(entry))}`;
    const headerMarkup = `<div style='display:flex; align-items:center'>
                  <span>${headerText}</span>
                  ${shareableLinkMarkup}
                </div>`;

    let scripturesFromChapterData = JChronologyData.getChapterRefs(entry);
    ...
    /**********************************************************************/
    /* 26-Aug-2026: $$$Jamie Changed so that the couple of lines which were
       running outside of the async block now run inside of it instead. */
    ...
    (async () => {
      ...
      for (const chapterRef of refs.split('; '))
      {
        ...
        let associatedText = await JFrameworkChapterSummaries.instance().getChapterSummary(chapterRef);
...
//	infoBox.scrollTop = 0;
  ...
  _setInfoBoxContentForStandardEvent (entry)
  {
    ...
    let scripturesFromChapterData = JChronologyData.getChaptersFromChapterAndYearData(entry);
    if (0 !== scripturesFromChapterData.length)
    {
    ...
    (async () => {
      ...
      for (const chapterThunk of chaptersFromChapterAndYearData)
      {
        ...
        let associatedText = await JFrameworkChapterSummaries.instance().getChapterSummary(chapterThunk.ref);
...
class _ClassCommon {
  ...
  static getArbitraryDateMarker() {
    return _ClassCommon._arbitraryDateMarker;
  } // The marker used when asked to select a date which does not exist in hte events list.
  static setArbitraryDateMarker(marker) {
    _ClassCommon._arbitraryDateMarker = marker;
  }
  ...
  static convertToRanges(input)
  {
    ...
    if ('string' === typeof input) input = input.split('; ');
    else input = input.map((sublist) => sublist[0]);
    ...
    for (let i = 1; i <= items.length; i++) {
      ...
      if (start.num === last.num) result.push(`${start.prefix}.${start.num}`);
      else result.push(`${start.prefix}.${start.num}-${last.num}`);
  ...
  static getColumnsContainer() {
    // The div which holds the whole of the lower part of the screen.
    if (null === _ClassCommon._columnsContainer)
      _ClassCommon._columnsContainer = document.getElementById(
  ...
  static _verticalDivIds = [
    'jchronologySelectedItemHighlighterHolder',
  ...
  static getVerticalDiv(id) {
    return this._verticalDivsAsObject[id];
  }
  static getVerticalDivs() {
    return this._verticalDivsAsList.map((x) => x.div);
  }
  static getVerticalDivIds() {
    return this._verticalDivIds;
  }
  static setVerticalDivs(verticalDivs) {
    this._verticalDivsAsList = verticalDivs;
    this._verticalDivsAsObject = Object.fromEntries(
      verticalDivs.map((o) => [o.id, o.div]),
    );
  }
  ...
  static getDurationHeaderContainer() {
    // The div at the top of the graphics area which gives headers for things only partially visible.
    if (null === _ClassCommon._durationHeaderContainer)
      _ClassCommon._durationHeaderContainer = document.getElementById(
  ...
  /**************************************************************************/
  static getDurationLinesContainer() {
    // The div which holds the timeline and associated graphics.
  ...
  static setSelectedItemHighlighter(div) {
    this._selectedItemHighlighter = div;
  }
  static getSelectedItemHighlighter() {
    return this._selectedItemHighlighter;
  }
  ...
  static getVerticalGapBetweenConsecutiveDurationLines() {
    if (null === this._verticalGapBetweenConsecutiveDurationLines)
      this._verticalGapBetweenConsecutiveDurationLines =
        _ClassCommon.getVariableSetting(
          '--verticalGapBetweenConsecutiveDurationLines',
        );
  ...
  static _initDummyRootResolver() {
    if (null !== this._dummyRootResolver) return;

    this._dummyRootResolver = document.getElementById('dummyRootResolver');
    this._dummyRootResolverMap = new Map();
    for (let attr of this._dummyRootResolver.attributes)
      if (attr.name !== 'id')
        this._dummyRootResolverMap.set(
          attr.name,
          getComputedStyle(this._dummyRootResolver)[attr.value],
        );
  }
  ...
  /**************************************************************************/
  static resetRootResolver() {
    this._dummyRootResolver = null;
  }

  /**************************************************************************/
  static getVerticalSpaceForDatesMinimumToAvoidOverlap() {
    return this._getFromRootResolver(
      'verticalSpaceForDatesMinimumToAvoidOverlap',
    );
  }
  static getVerticalSpacePerYearForAmDates() {
    return this._getFromRootResolver('verticalSpacePerYearForAmDates');
  }
  static getVerticalSpacePerYearForNonAmDates() {
    return this._getFromRootResolver('verticalSpacePerYearForNonAmDates');
  }
}
